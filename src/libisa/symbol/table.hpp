 ...
 * 
 * This table forms the inner node of a hierachical symbol table.
 ...
 * No symbols or tables are allowed to be deleted to prevent confusion when dealing with externals' definitions changing.
 * 
 * \tparam value_t An unsigned integral type that is large enough to contain the largest address on the target system.
 */
template<typename value_t>
...
public:
	 ...
	 * These methods make sure that the added table has the proper parent, and the parent has an owning reference to the child.
	 * \arg child A symbol table that is
	 * \sa symbol::insert_leaf
	 * \sa symbol::insert_branch
	 */
...
public:
	...
	//! Returns true if this table (not checking any other table in the hierarchy) contains a symbol with the matchign name.
