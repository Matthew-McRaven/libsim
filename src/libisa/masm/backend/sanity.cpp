...
bool fixup(std::shared_ptr<masm::project::project<uint16_t> >& project, 
	std::shared_ptr<masm::elf::code_section<uint16_t> >& section) {
	bool success = true;
	int end_count = 0, burn_count = 0;
	auto& ir_lines = section->body_ir.value().ir_lines;

	for(auto &line : ir_lines) {
		if(auto ptr = std::dynamic_pointer_cast<masm::ir::dot_burn<uint16_t>>(line); ptr) burn_count++;
		if(auto ptr = std::dynamic_pointer_cast<masm::ir::dot_end<uint16_t>>(line); ptr) end_count++;
	}
	/*
	...
	if(section->header.section_type == masm::elf::program_type::kOperatingSystem) {
		...
		std::list<std::shared_ptr<masm::ir::macro_invocation<uint16_t>>> macro_fixups;
		// Flag any line of code prior to a BURN as not generating object code.
		for(auto &line : ir_lines) {
			if(line->source_line < burn_source_index) {
				if(auto ptr = std::dynamic_pointer_cast<masm::ir::macro_invocation<uint16_t>>(line); ptr)
		...
		while(!macro_fixups.empty()) {
			...
			for(auto &line : macro->macro->body_ir.value().ir_lines) {
				if(auto ptr = std::dynamic_pointer_cast<masm::ir::macro_invocation<uint16_t>>(line); ptr)
					macro_fixups.push_back(ptr);
				line->emits_object_code = false;
			}
