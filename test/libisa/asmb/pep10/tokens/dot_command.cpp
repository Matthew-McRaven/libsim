...
TEST_CASE( "Dot Directives", "[masm::tokens]"  ) {
    
	using tokenizer_t = asmb::pep10::tokenizer<masm::frontend::lexer_t>;
	tokenizer_t lexer;
	SECTION("First 4 dot commands") {
		auto result = masm::frontend::tokenize<tokenizer_t>(".ADDRSS .ASCII .ALIGN .BLOCK", lexer);
	...
	SECTION("Second 4 dot commands") {
		auto result = masm::frontend::tokenize<tokenizer_t>(".burn .bYtE .ENd .Equate", lexer);
		CHECK(result.success);
		CHECK(result.rest.empty());
		CHECK(!result.error_message);
		CHECK(result.tokens.size() == 4);
	...
	SECTION("Third 4 dot commands") {
		auto result = masm::frontend::tokenize<tokenizer_t>(".EXPORT .SYCALL .USYCALL .WORD", lexer);
		CHECK(result.success);
		CHECK(result.rest.empty());
		CHECK(!result.error_message);
		CHECK(result.tokens.size() == 4);
