...
TEST_CASE("Test operand decoding for loading bytes", "[isa::pep10]")
{
	 ...
	 * 	Please note that the values loaded are 1-byte, but addressing modes which touch memory use 2-byte intermediate addresses.
	 * 	Operands are decoded according to the correct addressing mode.
	 * Tests do not need to ensure that each instruction's addressing modes are ordered correctly--other tests ensure this.
	 * To achieve this with a minimal number of instructions, tests are allowed to mutate registers directly.
	 * This reduces the number of instructions that must be validated before running these tests to include only LDWA.
	 */
